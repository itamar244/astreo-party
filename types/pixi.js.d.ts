declare module 'pixi.js' {
	...
	export class DisplayObject extends utils.EventEmitter
		...
		protected _mask: Graphics | Sprite | null;
	...
	export class Graphics extends Container {
		...
		protected _renderSpriteRect(renderer: SystemRenderer): void;
	...
	export class GraphicsRenderer extends ObjectRenderer {
		...
		protected updateGraphics(graphics: Graphics): void;
	...
	export namespace GroupD8 {
		...
		export function matrixAppendRotationInv(
			matrix: Matrix,
			rotation: number,
			tx: number,
			ty: number,
	...
	export interface ApplicationOptions extends RendererOptions {
		/**
		 * `true` to use ticker.shared, `false` to create new ticker. [default=false]
		 */
		sharedTicker?: boolean;

		/**
		 * `true` to use loaders.shared, `false` to create new Loader.
		 */
		sharedLoader?: boolean;
	...
	export interface WebGLRendererPlugins
	...
	export class Renderer extends SystemRenderer {
		...
		static __plugins: {
			[pluginName: string]: { new (renderer: Renderer): any };
		};

		static registerPlugin(
			pluginName: string,
			ctor: { new (renderer: Renderer): any },
		): void;
	...
	export namespace interaction {
		...
		export class InteractionManager extends utils.EventEmitter {
			constructor(
				renderer: CanvasRenderer | Renderer | SystemRenderer,
			...
			protected dispatchEvent(
				displayObject: Container | Sprite | TilingSprite,
				eventString: string,
				eventData: any,
			): void;
	...
	export class Loader extends utils.EventEmitter {
		...
		on(
			event: 'error',
			fn: (error: Error, loader: Loader, resource: LoaderResource) => void,
			context?: any,
		): this;
		on(
			event: 'load' | 'progress',
	...
	export namespace prepare {
		...
		export abstract class BasePrepare<UploadHookSource> {
			...
			protected findBaseTexture(item: DisplayObject, queue: any[]): boolean;

			protected findTexture(item: DisplayObject, queue: any[]): boolean;
	...
	export class Geometry {
		...
		addAttribute(
			id: string,
			buffer: Buffer | Array<number> | ArrayBuffer | ArrayBufferView,
			size?: number,
			normalised?: boolean,
			type?: number,
			stride?: number,
	...
	export namespace systems {
		export class FilterSystem extends System {
			...
			applyFilter(
				filter: Filter,
				inputTarget: RenderTexture,
				outputTarget: RenderTexture,
				clear?: boolean,
			): void;
	...
	export class BaseTexture extends utils.EventEmitter {
		...
		static fromBuffer(
			buffer: Float32Array | Uint8Array | Uint32Array,
			width: number,
			height: number,
			options: IBaseTextureOptions,
		): BaseTexture;
	...
	export namespace utils {
		...
		export function getResolutionOfUrl(
			url: string,
			defaultValue?: number,
		): number;
		...
		export function correctBlendMode(
			blendMode: number,
...
declare module '@pixi/graphics' {
	export { Graphics, GraphicsRenderer } from 'pixi.js';
}
